...
class SoundsApi(object):
    ...
    def sound_add(self, url, **kwargs):
        ...
        >>>     pprint(response)
        >>>
        >>> thread = api.sound_add(url, callback=callback_function)
        ...
        if ('url' not in params) or (params['url'] is None):
            raise ValueError("Missing the required parameter `url` when calling `sound_add`")

        resource_path = '/sounds'.replace('{format}', 'json')
        path_params = {}
